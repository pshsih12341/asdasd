...
const AvatarStackComponent = ({ max = 3, total, overlapSize = 's', size, children, className, renderMore, }) => {
    ...
    React.Children.forEach(children, (child) => {
        ...
        const item = React.createElement(AvatarStackItem, { key: visibleItems.length }, child);
    ...
    React.createElement("ul", { className: b({ 'overlap-size': overlapSize }, className), role: 'list' },
        hasMoreButton ? (React.createElement(AvatarStackItem, { key: "more-button" }, renderMore ? (renderMore({ count: moreItems })) : (React.createElement(Avatar, { text: `+${moreItems}`, "aria-label": i18n('more', { count: moreItems }), size: size })))) : null,
